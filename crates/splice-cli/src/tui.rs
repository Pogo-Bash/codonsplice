    ...
    fn welcome() -> Self {
        OutputPane {
            ...
            lines: vec![
                ...
                Line::from("Ctrl+D: bytecode   Ctrl+A: AST   Tab: focus   F1: help   Ctrl+Q: quit"),
            ],
        }
    }
...
impl App {
    ...
    fn draw(&self, frame: &mut Frame) {
        ...
        // Split the editor column to show a VARIABLES panel when the query
        // references $vars.
    ...
    fn draw_header(&self, frame: &mut Frame, area: Rect) {
        ...
        let hints = Line::from(Span::styled(
            "Ctrl+Enter run · Ctrl+D bytecode · Ctrl+A AST · Tab focus · F1 help · Ctrl+Q quit",
    ...
    fn draw_editor(&self, frame: &mut Frame, area: Rect) {
        ...
        let text: Vec<Line> = self.lines.iter().map(|l| Line::from(l.clone())).collect();
...
mod tests {
    ...
    #[test]
    fn run_query_populates_output() {
        let mut app = App::new();
        app.run_query(); // default query compiles
        assert!(!app.output.lines.is_empty());
    }

    #[test]
    fn typing_inserts_into_editor() {
        let mut app = App::new();
        app.lines = vec![String::new()];
        app.cursor_row = 0;
        app.cursor_col = 0;
        app.on_key(press(KeyCode::Char('X')));
        assert_eq!(app.lines[0], "X");
    }
}
